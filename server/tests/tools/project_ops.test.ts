...
const dbState = {
  projects: new Map<string, { id: string; user_id: string; name: string; repo_path: string; files_path?: string }>(),
};
...
describe('delete_project', () => {
  it('removes the project record without deleting files when delete_files is false', async () => {
    ...
    dbState.projects.set('p1', { id: 'p1', user_id: userId, name: 'api', repo_path: repoDir });
  ...
  it('returns an error for a project not belonging to the user', async () => {
    dbState.projects.set('p1', { id: 'p1', user_id: 'other-user', name: 'api', repo_path: '/fake' });
    const result = await deleteProject({ project_id: 'p1', delete_files: false }, userId, 'exec-1');
  ...
  it('returns a cancellation message when the user rejects', async () => {
    ...
    dbState.projects.set('p1', { id: 'p1', user_id: userId, name: 'api', repo_path: '/fake' });
